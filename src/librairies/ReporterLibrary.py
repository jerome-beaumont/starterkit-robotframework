...
class ReporterLibrary(object):
    ...
    def __init__(self):
        ...
        self._logger = StepsLogger(
            tz="UTC", ms3=True, color=True, emoji=True, colored_console=False
        )
    ...
    def get_audit_data(attrs: dict) -> dict:
        ...
        """
        suite_name, test_name = ReporterLibrary.extract_suite_and_test(
            attrs["longname"]
        )
    ...
    def audit_ndjson(audit_trail: dict) -> None:
        ...
        try:
            ...
            output_dir = "."
        file_path = os.path.join(output_dir, audit_trail["source"])
    ...
    def _end_test(self, name, attrs):
        ...
        (
            self._logger.test(
                f"Test '{name}' terminé avec le statut: {audit_trail['test_status']} en {audit_trail['test_elapsed']} ms."
            )
            if audit_trail["test_status"] == "PASS"
            else self._logger.error(
                f"Test '{name}' échoué avec le message: {audit_trail['test_message']}",
                category="TEST",
            )
        )
    ...
    def _end_keyword(self, name, attrs):
        ...
        known_types = {"page", "service", "socle", "etape"}
